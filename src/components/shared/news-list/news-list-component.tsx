...
const NewsList = ({
    ...
    return (
        ...
        >
            {filterable && (
                <div className='flex-1 animate-fade-in p-2 flex flex-col-reverse md:flex-row gap-2 mt-4 max-w-full justify-between'>
                    <div
                        data-cy={`${siteName ?? 'all'}-news-filters`}
                        className='animate-fade-in flex-1 flex flex-col w-full p-2 default-border rounded-lg shadow bg-white/90 dark:bg-zinc-900/90'
                    >
                        <button
                            type='button'
                        ...
                        <div className='p-2 md:p-4 absolute left-0 top-30 md:top-16 w-full'>
                            <div
                                data-cy={`${siteName ?? 'all'}-news-outlet-filters`}
                                className='p-4 animate-fade-in w-full rounded-lg flex flex-col gap-2 bg-white/90 dark:bg-zinc-900/90 default-border shadow backdrop-blur-sm z-10'
                                hidden={!filtersOpen}
                            >
                                <div className='rounded-lg p-2 default-border'>
                                    <span>Outlet Selection:</span>
                                    {outlets &&
                    ...
                    <div className='flex-1 animate-fade-in w-full p-2 default-border rounded-lg shadow bg-white/70 dark:bg-zinc-900/70'>
            ...
            {loaded === true && !expanded && (
                <div className='animate-fade-in text-left flex flex-col w-full justify-center h-120 default-border rounded-lg relative shadow-inner'>
                    <h2 className='uppercase absolute top-0 left-0 px-2 ml-2 -mt-2 text-sm bg-white/40 dark:bg-zinc-900/10 backdrop-blur-md rounded-lg'>
                        {siteName}
                    </h2>
            ...
            {loaded === true && expanded && (
                <div className='animate-fade-in text-left flex flex-col w-full justify-center'>
                    <div
                        data-cy={`${siteName ?? 'all'}-news-items`}
                        className={`my-3 flex flex-col gap-4 p-1 px-4 md:px-10`}
                    >
                        {filteredArticles.map(
                            ({
                                ...
                                <a
                                    ...
                                    rel='noreferrer'
                                    target='_blank'
                                    className='text-base default-bg hover default-border flex flex-col items-start rounded-lg p-2 md:p-4 shadow'
                                >
